...
void Admin::on_pushButton_9_clicked()
{
    ...
    QMessageBox msg;
    msg.setText(name+"ni avtomobillar ro'yxatidan o'chirmoqchimisiz?");
    msg.setStandardButtons(QMessageBox::Ok | QMessageBox::Cancel);
    int ret = msg.exec();
    switch (ret) {
        case QMessageBox::Ok:{
            QSqlQuery del;
            QString sqld="DELETE FROM cars WHERE id="+id;
            del.prepare(sqld);
            if(del.exec())
                QMessageBox::information(this,("!"),"O'chirildi",QMessageBox::Ok);
...
void Admin::on_pushButton_10_clicked()
{
    ...
    QString rusumi = ui->lineEdit_9->text().trimmed();
    QString soni = ui->lineEdit_10->text().trimmed();
    QString narxi = ui->lineEdit_11->text().trimmed();
    QString id = ui->lineEdit_12->text();

    QString sqlu = "UPDATE cars SET name='"+name+"', rusumi='"+rusumi+"', soni="+soni+",narxi="+narxi+" WHERE id="+id;
    qu.prepare(sqlu);
    qu.exec();
...
void Admin::on_pushButton_11_clicked()
{
    ...
    QString rusumi = ui->lineEdit_14->text().trimmed();
    QString soni = ui->lineEdit_15->text().trimmed();
    QString narxi = ui->lineEdit_16->text().trimmed();
    if(name.size()>0&&rusumi.size()>0&&soni.size()>0&&narxi.size()>0){
        QString sql = "INSERT into cars (name,rusumi,soni,narxi) VALUES ('"+name+"','"+rusumi+"',"+soni+","+narxi+")";
        q.prepare(sql);
        if(q.exec()){
            QMessageBox::information(this,"ok","Avtomobil qo'shildi!",QMessageBox::Ok|QMessageBox::Cancel);
